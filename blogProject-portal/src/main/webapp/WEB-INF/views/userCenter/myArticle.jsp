					...
					<div class="tech-no">
						...
						<c:forEach items="${page.items}" var="article">
							<div class="tc-ch wow fadeInDown" data-wow-duration=".8s"
								...
								<h3>
									<a href="${ctp }/article/getArticle?id=${article.id }">${article.articleName }</a>
								</h3>
								<h6>
									BY <a href="#">${user.userName } </a>JULY 10 2016.
								</h6>
								<p>${article.articleSimplecontent }<a
										href="${ctp }/article/getArticle?id=${article.id }">...</a>
								</p>

								<a href="${ctp }/article/getArticle?id=${article.id }">继续阅读</a>
